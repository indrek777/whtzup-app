...
makeRequest('/health')
  .then(response => {
    ...
    const testEvent = {
      name: 'Quick Fix Test Event',
      description: 'Test event for quick fix',
      category: 'other',
      venue: 'Test Venue',
  ...
  .then(response => {
    ...
    console.log('📊 Test upload response data:', JSON.stringify(response.data, null, 2));
          ...
          setTimeout(async () => {
            try {
              ...
              const dockerEvent = {
                ...
                createdBy: String(appEvent.createdBy || 'Quick Fix Script').trim()
              };
  ...
  .catch(error => {
    ...
    console.log('4. Try again: node quick-fix.js');
  });
