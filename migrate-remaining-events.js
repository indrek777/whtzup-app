...
async function main() {
  console.log('🔄 Starting migration of remaining events...');
  ...
  // Process only first 100 events to test
  const eventsToProcess = filteredEvents.slice(0, 100);
  console.log(`🧪 Processing first ${eventsToProcess.length} events as test`);
  
  // Process events in batches
  const batchSize = 5;
  for (let i = 0; i < eventsToProcess.length; i += batchSize) {
    const batch = eventsToProcess.slice(i, i + batchSize);
    console.log(`📦 Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(eventsToProcess.length / batchSize)} (${i + 1}-${Math.min(i + batchSize, eventsToProcess.length)})`);
    ...
    const currentBackendCount = await getExistingEvents();
    console.log(`📊 Current backend count: ${currentBackendCount.length}`);
  }
